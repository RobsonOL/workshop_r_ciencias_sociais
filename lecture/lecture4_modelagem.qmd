...
subtitle: "Aula 04 - Modelagem de Dados"
...
## Estatística: Correlação {.smaller}
...
## Estatística: Distribuição Normal {.smaller}
...
## Estatística: Diferença de Médias {.smaller}
...
```{r lm1, eval=FALSE, `code-line-numbers`="1"}
...
```{r lm-predict1, eval=FALSE, `code-line-numbers`="3"}
equacao <- "log(salario + 1) ~ sexo + idade + escolaridade + meso_trab + semiarido_trab"
...
```{r lm-predict2, eval=TRUE, `code-line-numbers`="4"}
equacao <- "log(salario + 1) ~ sexo + idade + escolaridade + meso_trab + semiarido_trab"
...
$$
\log(\text{Salário} ) = 5,78 - 0,13 \text{ Mulher} + 0,02 \text{ Idade} + \\[0.1in] 0.10 \text{ Escolaridade} - 0,07 \text{ Borborema} + \\[0.1in] 0.20 \text{ Zona Mata} -  0,09 \text{ Sertão} + \\[0.1in] 0,08 \text{Semiarido}
$$
