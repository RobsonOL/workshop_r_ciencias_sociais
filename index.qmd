...
Vou apresentar uma lista (não exaustiva) de modelagem de dados que podem ser desenvolvidos nas ciências sociais:
...
## Estatísticas Básica: Estatísticas Descritivas
...
salario_pb |> 
  ...
  geom_histogram(breaks = seq(0,10000, by = 100),
                 alpha = 0.3) +
  labs(x = "Salário", y = "", title = "Histograma do Salário Nominal na Paraíba (2013)",
...
resultado <- lm(equacao, data = salario_pb) 
resultado |> 
  broom::tidy()
```
