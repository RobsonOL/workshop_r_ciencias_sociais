...
E agora podemos ver o que foi feito:
...
E salvar esses resultados como um arquivo \*.csv\*:

```{r eval=FALSE}
despesas_saude_ano |> 
  write_csv("despesas_saude_ano.csv")
```
...
> Vamos prever o valor futuro do consumo de energia residencial na Paraíba. Os dados podem ser obtidos no [link.](https://basedosdados.org/dataset/br-mme-consumo-energia-eletrica?bdm_table=uf)
...
```{r, eval=FALSE, `code-line-numbers`="2"}
energia_residencial_pb <- energia |> 
  filter(tipo_consumo == "Residencial") |> 
  filter(sigla_uf == "PB") |> 
...
```{r}
energia_residencial_pb |> 
  plot_time_series(data, consumo, .title = "Consumo de Energia Residencial na Paraíba")
...
```{r}
receita_energia <- recipe(consumo ~ data, training(energia_treinamento_teste))
```
...
workflow_arima <- workflow() %>% 
  ...
  fit(training(energia_treinamento_teste))

previsao_energia <- workflow_arima |> 
  modeltime_calibrate(new_data = testing(energia_treinamento_teste)) |> 
  modeltime_forecast(new_data = testing(energia_treinamento_teste),
